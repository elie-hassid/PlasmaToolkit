...
def _read_block(fp, has_arg=True):
    ...
    has_arg indicates whether we have to read an argument line
    """
...
#
# Specialized function for each keyword. They all return dictionaries with the
# relevant attributes.
# 
...
def _read_excitation(fp):
    ...
    parts = arg.split()
    if len(parts) > 1:
        threshold, weight_ratio = float(parts[0]), float(parts[1])
